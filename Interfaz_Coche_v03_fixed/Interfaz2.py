...
[sg.Checkbox('Conectar', default=False,enable_events = True,background_color="white", text_color = "black")]
...
# Create the window
window = sg.Window("5G RobotCar", layout,size=(1000,800),element_justification='c',background_color="white")

while True:
    event, values = window.read()
    try:
        sendCheck(str(values.get(2)).encode("utf-8"))
    except:
        if event == sg.WIN_CLOSED:
            break
        print("Volante Desconectado")
        continue
