...
def get_speed(input_fpedal, input_brake, forward, backward):
    # booleans select the direcction
    brake = (input_brake+1)/2
    speed = (input_fpedal+1)/2
    if forward:
        speed = 90+velocity*speed-velocity*brake
        if speed < 90:
            speed = 90
    elif backward:
        speed = 90-velocity*speed+velocity*brake
    ...
    def listen(self):
        ...
        while True:
            for event in pygame.event.get():
                ...
                current_speed = get_speed(self.axis_data.get(4), self.axis_data.get(
                    3), self.button_data.get(5), self.button_data.get(4))
